...
def test_classification_of_named_items_from_the_real_snapshot() -> None:
    ...
    assert RAVAGER_EMBLEM not in stats and classify_item(stats.get(RAVAGER_EMBLEM)) == "unknown"
...
def _elise_db(path: Path) -> Database:
    """Elise: 3 defensive boards, 3 offensive boards (one with the Ravager
    emblem), 1 unitemized board."""
    db = Database(path)
    for i, items in enumerate(([WARMOG, GARGOYLE], [WARMOG, GARGOYLE, CLAW], [GARGOYLE, VISAGE])):
        db.ingest_match(_board(f"DEF{i}", ELISE, items, placement=6, tier=3 if i == 0 else 2))
    for i, items in enumerate(([RAVAGER_EMBLEM, GUINSOO], [RAVAGER_EMBLEM, GUINSOO, TITAN], [GARGOYLE, GUINSOO, TITAN])):
...
def test_downstream_evidence_uses_only_qualifying_boards(tmp_path: Path) -> None:
    ...
    assert all(WARMOG not in s for s in package_sets) and len(package_sets) == 3
    individual = {a.key for a in packages["items"]}
    assert WARMOG not in individual and GUINSOO in individual
...
def test_current_sample_regression_tank_spam_removed_offmeta_carry_kept(
    ...
    with Database(tmp_path / "regression.sqlite3") as db:
        for i in range(12):  # the frontliner everyone slams tank items on
            db.ingest_match(_board(f"WALL{i}", "TFT99_Wall", [WARMOG, GARGOYLE] + ([CLAW] if i % 2 else []), placement=4))
        for i in range(6):  # the same kind of unit, built as a carry
            db.ingest_match(_board(f"FLIP{i}", "TFT99_OffMeta", [GARGOYLE, GUINSOO, TITAN], placement=2))
        for i in range(4):
...
def _sql_matches_python(db: Database) -> None:
    ...
    assert expected == {"TFT99_P0", "TFT99_P4", "TFT99_P5", "TFT99_P7", "TFT99_P8"}
