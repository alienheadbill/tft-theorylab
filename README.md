...
- **KNOWN_UNLISTED** -- a known completed item no role recommends (Steadfast Heart, Crownguard, Thief's Gloves, Tactician's items, artifacts). This is **not** "tank"; it only means Riot gives no role evidence for it.
- **UNKNOWN** -- not resolvable: ids missing from the snapshot (a new patch), unresolved aliases, and Set 18 trait emblems (`DA_18_Emblem*`, e.g. Ravager Emblem = `DA_18_EmblemSlayer`), which have no counterpart in the recommendation namespace.

**Rule.** A board counts when any completed item is DAMAGE, MIXED or UNKNOWN (unknown stays conservative: hiding an unusual build is worse than a false positive). TANK and KNOWN_UNLISTED items alone never prove carry intent. So Spirit Visage + Steadfast Heart (the production Leona board that ranked #1 because PR #21 read Steadfast Heart's raw `CritChance` as offensive), Warmog's + Gargoyle and Crownguard + Warmog's are not carry observations, while Titan's + Sterak's, Ravager Emblem + Guinsoo's and an unknown emblem + Warmog's are -- per board, never per champion, so an off-meta tank-to-carry conversion (Elise with an emblem + Guinsoo's) stays discoverable. Star level plays no part.
...
**Snapshot and provenance.** `src/tftlab/data/item_intent.json` is committed and read offline (analytics and the web app never call CommunityDragon). It keeps every role (object key, UI name key, UI name, family, recommended items) and, for each of the 235 completed items, the Riot items it resolved to, exactly which Tank and non-Tank roles recommend them, and the derived intent -- so "why is Warmog's TANK?" is answered by the file (recommended by `ADTank`, `APTank`, `HTank`, by no non-Tank role). The opt-in live test `test_committed_item_intent_matches_live_set` fails and prints a fresh snapshot whenever Riot changes a role or a recommendation; `test_riot_role_vocabulary_is_classified_and_resolves` checks every role object and that every recommended item is a known id.
