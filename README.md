...
Riot Match-V1 has started returning a masked, unparseable `game_version` for some matches (`"TFT Unreal Version ?.?.?.?"`), with no major.minor version left in the string at all. `tftlab.unreal_patch` resolves the real client patch for these from `game_datetime` alone, via `UNREAL_PATCH_REGISTRY` -- a registry of known patch-deployment cutover timestamps, kept entirely separate from `balance_window`'s mid-patch registry above (one answers "which client patch"; the other then answers "which half of that patch").

**`UNREAL_PATCH_REGISTRY` currently ships empty.** This sandbox's network egress can't reach Riot's patch notes to confirm real deployment timestamps, and an invented timestamp would silently mislabel real production matches with more confidence than the data deserves. Until real, verified entries are added:

- A masked-Unreal match resolves to the explicit `UNRESOLVED_UNREAL_PATCH` sentinel (`"unreal-unresolved"`), never a fake shared patch bucket and never the raw masked string.
...
- Run `tftlab validate-live-data` and read its diagnostics (raw `game_version` distribution, resolved patch distribution, balance-window distribution, earliest/latest `game_datetime` -- store-wide, not scoped to one window) to see the actual timestamp range needing a cutover.
- Add a verified `UnrealPatchCutover(client_patch=..., starts_at=<epoch_ms>, verified=True, source=...)` entry to `UNREAL_PATCH_REGISTRY` in `src/tftlab/unreal_patch.py` once you have the real deployment date from Riot's patch notes. Every affected row -- already-ingested matches included -- picks up the correct patch automatically on the next `Database` connect; no separate backfill command or data wipe is needed.
