...
4. Run a small, safe first ingest:
   ...
   Challenger-only by default. `--sampling high_elo` (or the older `--include-master`) draws seeds from Challenger, Grandmaster and Master instead -- see "Data population and seed sampling" below. Prints a report: sampling mode, requested and actual seed players with the count from each ladder tier, requested histories per seed, match-ID references returned before dedupe, unique match IDs after dedupe, matches skipped as already stored, fetched, inserted, failed requests, non-ranked-queue matches skipped, derived rates (in-run overlap, already-stored duplicates, inserted per seed, new-match yield), balance windows found, and total participants now stored. A single match's fetch failing doesn't abort the batch -- it's counted in "failed requests" and the run continues; a single seed's history request failing is likewise counted and skipped (the command exits non-zero only if every seed's history failed).
...
## Data population and seed sampling

**Data population:** recent high-Elo NA standard Ranked TFT matches reached through selected Challenger / Grandmaster / Master ladder players. This is not "all TFT games" and is not globally representative -- it is whatever those seed players played recently, restricted to the ranked queue (`RANKED_TFT_QUEUE_ID`).

**Analytical search space:** everything observed inside that population -- every champion, every qualifying carry, every partner, item package and trait breakpoint on every sampled board. Discovery is not limited to the comps saved in My Experiments or to any named champions; the examples there (Kha'Zix, Cassiopeia / Fiddlesticks, Caitlyn, ...) are test cases, not a search list.

Seed selection (`tftlab.sampling.select_seeds`) knows nothing about champions, items or comps -- it only picks whose match histories to read, deterministically:

1. Fetch each ladder tier the mode asks for, once (`challenger` mode fetches only Challenger; `high_elo` fetches Challenger, Grandmaster and Master). A player listed in two tiers counts once, in the higher tier.
2. Split the requested seed count between tiers by fixed weights (`high_elo` is 4:3:3) using the largest-remainder method, ties to the higher tier: 50 seeds -> 20 / 15 / 15, 25 -> 10 / 8 / 7. If a tier has fewer players than its share, the unused seats are re-split among the tiers with room, so the total is met whenever enough players exist.
3. Within a tier, rank players by League Points (then PUUID, so Riot's response order never matters) and take seeds at evenly spaced ranks across the whole tier rather than only its top. Players close in LP share lobbies, so spreading seeds reaches more distinct games and represents the whole tier.

Match IDs are then deduplicated across all seed histories before any body is fetched (one lobby is usually in several seeds' histories and becomes one match), and IDs already stored are skipped without being fetched again.
...
- `tftlab ingest-riot [--players N] [--matches-per-player N] [--sampling challenger|high_elo] [--current-trusted-window | --start-time ISO8601] [--allow-degraded-costs]` -- see "Run on live Riot data" above.
...
**Inputs** (Run workflow form): `players` (seed players, 1-100, default 10), `matches_per_player` (recent matches per seed, 1-10, default 5 -- the CLI's deeper 100-match maximum is deliberately not exposed here, since more players beats deeper histories of the same players), and `expanded_high_elo` (default off = Challenger only; on = the stratified Challenger / Grandmaster / Master sampling above). The defaults reproduce the original conservative 10 x 5 Challenger run. It has **no** `push`, `pull_request`, or `schedule` trigger -- it only ever runs when someone explicitly starts it, and a failure in any step (bad key, unreachable CommunityDragon, unreachable database, a severe integrity issue) stops the run there rather than continuing partway.
...
**Production safety checks:** before touching Riot or the database, a first `Validate production configuration` step fails the run (with a static error message, never a secret value) if the selected branch isn't `main`, if `RIOT_API_KEY`/`DATABASE_URL` is missing, if `DATABASE_URL` doesn't start with `postgres://`/`postgresql://` -- this exists specifically so the ingest CLI's normal local-SQLite fallback can never be silently used in production -- or if an input is out of bounds (`players` 1-100, `matches_per_player` 1-10, `expanded_high_elo` true/false). Inputs reach the shell only as environment variables. The workflow also declares `permissions: contents: read` (it never needs to write to the repo), a 60-minute job timeout, and a fixed `concurrency` group so two manually-triggered runs can never ingest into production at the same time (a second run queues rather than cancelling the first).
